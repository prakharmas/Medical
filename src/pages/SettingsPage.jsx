...
export default function SettingsPage() {
  ...
  const setDefaultTemplate = async (id) => {
    ...
    setSetDefaultError("");
    try {
      const res = await editPadTemplateDefault({ uid: id, is_default: true });
      if (res.data?.success) {
        setTemplateList((p) =>
          p.map((t) => ({ ...t, isDefault: t.id === id }))
        );
      } else {
        setSetDefaultError(res.data?.reason || "Failed to set default template.");
      }
    } catch (err) {
      setSetDefaultError(
            ...
            {viewTemplateLoading ? (
              ...
              <div className="settings-view-body">
                ...
                <div className="settings-view-info">
                  ...
                  {viewTemplate.details.template_info && (
                    <>
